...
Технологический стек (Рекомендуемый)
    ...
    Язык и сервер:
        TypeScript/Node.js (рекомендуется для backend и serverless функций)
    Frontend/Backend:
        Vercel (развёртывание фронтенда и serverless backend)
    База данных:
        Supabase (PostgreSQL + аутентификация)
    Интеграции:
        Stripe (биллинг)
        Calendly (запись на демонстрацию)
        Elevenlabs API (voice-to-voice и анализ)
        OpenAI API (GPT-4o для анализа)
    MCP:
        Использование MCP для интеграции с Vercel и Supabase (настройка через Cursor)
    DevOps:
        CI/CD через Vercel, автоматизация деплоя
...
Требования к MVP
    ...
    Интеграция с Calendly
    Деплой на Vercel, хранение данных в Supabase
    Весь backend и бизнес-логика реализованы на TypeScript/Node.js
```
